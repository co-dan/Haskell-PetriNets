...
data NodeMap a = NodeMap { map :: FiniteMap a Node,
                           key :: Int
                         }
               deriving Show
...
-- | Lookup a node, if present
lookupNode :: (Ord a) => NodeMap a -> a -> Maybe (LNode a)
lookupNode (NodeMap mp _) a = fmap (,a) (lookupFM mp a)
...
lookupNodeM :: (Ord a, DynGraph g) => a -> NodeMapM a b g (Maybe (LNode a))
lookupNodeM = liftN2' lookupNode 
